...
export default function ProjectGallery ({ files, setFiles, cover, setCover }: MyProps) {
  ...
  const onDrop = (acceptedFiles: File[]) => {
    setFiles([...files, ...acceptedFiles.map(file =>
      Object.assign(file, { preview: URL.createObjectURL(file), aspectRatio: "4/3", id: uuidv4() })
    )])
  }

  function deleteFile (toDelete: number) {
    console.log(files[toDelete])
    setFiles(prevFiles => {
  ...
  return (
    <div className="ml-2">
      ...
      <section className="lancr-add-edit-sect w-5/6 border rounded-md">
        ...
        <Masonry breakpointCols={2} className="my-masonry-grid py-10 px-6" columnClassName="my-masonry-grid_column">
          {files.map((file, idx) => (
            <div key={file.id} className={`${cover === idx ? "ring ring-offset-4 ring-purple-600 rounded-lg" : ""} mb-3 w-[300px]`}>
              <div style={{aspectRatio: `${file.aspectRatio}` }} className={`overflow-hidden mx-auto rounded relative shadow-md border`}>
                ...
                {cover !== idx && <div onClick={() => {
                  setCover(idx)
                  setFiles(prev =>prev.map((f, i) =>  i === idx ? { ...f, aspectRatio: "4/3" } : f))
              ...
              <div className="mt-3 mx-2 flex justify-around items-center border border-gray-500 shadow-sm rounded-lg py-1">
                ...
                <div className="flex gap-1 items-center">
                  <select disabled={cover === idx ? true : false} value={file.aspectRatio} onChange={(e) => setFiles(prev => prev.map((f, i) =>  i === idx ? { ...f, aspectRatio: e.target.value } : f))} className={`cursor-pointer border-2 border-gray-300 rounded-md outline-none focus:border-purple-600`} name="aspect-ratio" id="aspect-ratio">
